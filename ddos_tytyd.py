...
def put():
    usage()
    print(f"""
{putih}1{R}.{W}DDOS-v1 {biru}({G}Low{biru}) {biru}({G}rendah{biru})
...
""")
    ...
    if put == "1":
        v1()
    if put == "2":
        os.system("cd setup && python v2.py")
    if put == "3":
        os.system("xdg-open https://https://www.instagram.com/ammarexecuted/")
        tanya()
    if put == "4":
        os.system("xdg-open https://chat.whatsapp.com/GRZoMoLPNJA22TR7WINpMY")
        tanya()
    else:
        sys.exit(f"{W}[{R}!{W}] Masukin Yang Benar Kak{abu}...")
